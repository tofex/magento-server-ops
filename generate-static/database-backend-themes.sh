...
if [[ ${magentoVersion:0:1} == "2" ]]; then
  ...
  mysql -h"${databaseHost}" -P"${databasePort}" -u"${databaseUser}" "${databaseName}" -s -N -e "SELECT DISTINCT(theme_path) FROM theme WHERE area = \"adminhtml\" AND theme_id NOT IN (SELECT DISTINCT(parent_id) FROM theme WHERE area = \"adminhtml\" AND parent_id IS NOT NULL);"
fi
