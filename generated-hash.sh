  ...
  if [[ -n "${webServer}" ]]; then
    ...
    if [[ "${type}" == "local" ]]; then
      ...
      if [[ "${quiet}" == 1 ]]; then
        ...
        echo "${server}:${generatedHash}"
      else
        echo "--- Generating generated files hash on local server: ${server} ---"
        "${currentPath}/generated-hash-local.sh" -w "${webPath}" -u "${webUser}" -g "${webGroup}"
      fi
    fi
  fi
done
